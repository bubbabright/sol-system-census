...
function TreeBranch({ node, depth, selected, onSelect, matchSet, typeFilter, expanded, toggle }) {
    ...
    <div className="branch">
      ...
      {isExpanded && hasKids && (
        <div className="children">
          {node.children.map(c => (
            <TreeBranch key={c.id} node={c} depth={depth + 1} selected={selected} onSelect={onSelect} matchSet={matchSet} typeFilter={typeFilter} expanded={expanded} toggle={toggle}/>
          ))}
        </div>
      )}
    </div>
...
function App() {
  ...
  useEffect(() => {
    const body = selected ? {
      name: selected.name, type: selected.type, tldr: selected.tldr,
      mass: selected.mass, diameter: selected.diameter,
      orbit: selected.orbit, discovered: selected.discovered,
    } : null;
      ...
      <main className="layout">
        <section className="catalog">
          {groupBy === "binding" && (
            <>
              {treeRoots.map(root => (
                <TreeBranch key={root.id} node={root} depth={0} selected={selectedId} onSelect={selectBody} matchSet={matchSet} typeFilter={typeFilter} expanded={expanded} toggle={toggleExpand}/>
              ))}
              {/* Interstellar bucket */}
              {(() => {
