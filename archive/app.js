...
function buildTree(bodies) {
  ...
  bodies.forEach(b => {
    ...
    } else if (b.parent !== "interstellar") {
      // No resolvable parent = root. Bodies parented to "interstellar" are
      // rendered separately under the virtual Interstellar Space bucket.
...
function Controls({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "search",
    id: "search-input",
    placeholder: "Search bodies, e.g. Europa, Voyager, Halley\u2026 (press / to focus)",
    value: search,
...
function TreeBranch({
  ...
  const isMatch = (!matchSet || matchSet.has(node.id)) && typeFilter.has(node.type);
  return /*#__PURE__*/React.createElement("div", {
    className: "branch"
  }, /*#__PURE__*/React.createElement("div", {
    className: "rowwrap"
...
function App() {
  ...
  const [selectedId, setSelectedId] = useState(() => {
    try {
      const raw = localStorage.getItem("solcensus-state");
      if (raw) {
        const saved = JSON.parse(raw);
        if (saved.selectedId) return saved.selectedId;
      }
    } catch (e) {}
    return "sun";
  });
  const [expanded, setExpanded] = useState(() => {
    try {
      ...
      if (raw) {
        const saved = JSON.parse(raw);
        if (saved.expanded && saved.expanded.length) return new Set(saved.expanded);
      }
    } catch (e) {}
    return new Set(["sun", "earth", "mars", "jupiter", "saturn", "uranus", "neptune", "pluto", "interstellar"]);
  });
  const {
    byId,
    roots
  ...
  const matchSet = useMemo(() => {
    ...
    const direct = window.BODIES.filter(b => b.name.toLowerCase().includes(q) || b.id.toLowerCase().includes(q));
  ...
  // Persist selection + expanded state
  useEffect(() => {
    try {
      localStorage.setItem("solcensus-state", JSON.stringify({
        selectedId,
        expanded: [...expanded]
      }));
    } catch (e) {}
  ...
  // Ordered list of currently visible row ids (matches on-screen order), for keyboard nav + prev/next
  const visibleOrder = useMemo(() => {
    const out = [];
    if (groupBy === "binding") {
      function walk(node) {
        out.push(node.id);
        if (expanded.has(node.id) && node.children && node.children.length) {
          node.children.forEach(walk);
        }
      }
      treeRoots.forEach(walk);
      const visibleKids = interstellarNode.children.filter(c => typeFilter.has(c.type) && (!matchSet || matchSet.has(c.id)));
      if (visibleKids.length && expanded.has("interstellar")) {
        visibleKids.forEach(k => out.push(k.id));
      }
    } else if (groupBy === "type") {
      TYPE_ORDER.filter(t => typeFilter.has(t)).forEach(t => {
        flatVisible.filter(b => b.type === t).forEach(b => out.push(b.id));
      });
    } else {
      flatVisible.forEach(b => out.push(b.id));
    }
    return out;
  }, [groupBy, treeRoots, expanded, interstellarNode, typeFilter, matchSet, flatVisible]);

  function scrollRowIntoView(id) {
    requestAnimationFrame(() => {
      const rowEl = document.querySelector('[data-row-id="' + id.replace(/"/g, '') + '"]');
      if (!rowEl) return;
  ...
  function stepSelection(dir) {
    ...
    const curIdx = selectedId ? visibleOrder.indexOf(selectedId) : -1;
    let nextIdx;
    if (curIdx < 0) nextIdx = 0;
    else nextIdx = Math.max(0, Math.min(visibleOrder.length - 1, curIdx + dir));
    const nextId = visibleOrder[nextIdx];
    const b = byId[nextId];
    if (b) {
      selectBody(b);
  ...
  // Keyboard shortcuts: / focus search, Esc clear/close, arrows navigate + expand/collapse
  useEffect(() => {
    function onKey(e) {
      ...
      if ((e.key === "ArrowRight" || e.key === "ArrowLeft") && groupBy === "binding" && selectedId) {
        const node = byId[selectedId];
        if (!node || !node.children || node.children.length === 0) return;
        e.preventDefault();
        setExpanded(prev => {
          const n = new Set(prev);
          if (e.key === "ArrowRight") n.add(selectedId);else n.delete(selectedId);
          return n;
        });
        return;
      }
  ...
  }, [visibleOrder, selectedId, byId, search, groupBy]);

  // Broadcast the selected body for the "Ask the Professor" widget (worker/public/widget.js)
  useEffect(() => {
    const body = selected ? {
      name: selected.name,
      type: selected.type,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    hasPrev: !!selectedId && visibleOrder.indexOf(selectedId) > 0,
    hasNext: !!selectedId && visibleOrder.indexOf(selectedId) >= 0 && visibleOrder.indexOf(selectedId) < visibleOrder.length - 1
